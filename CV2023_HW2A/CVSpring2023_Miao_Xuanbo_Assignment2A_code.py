...
def FeatureMeasure2Points(R, npoints, threshold=0.01):
    ...
    num_labels, labels, stats1, centroids1 = cv2.connectedComponentsWithStats(
        mask_dilate1)
    num_labels, labels, stats2, centroids2 = cv2.connectedComponentsWithStats(
        mask_dilate2)
    num_labels, labels, stats3, centroids3 = cv2.connectedComponentsWithStats(
        mask_dilate3)
    num_labels, labels, stats4, centroids4 = cv2.connectedComponentsWithStats(
        mask_dilate4)

    # Remove the background label
    centroids_ = np.concatenate(
        (centroids1[1:], centroids2[1:], centroids3[1:], centroids4[1:]), axis=0)
    stats_ = np.concatenate(
        (stats1[1:], stats2[1:], stats3[1:], stats4[1:]), axis=0)
    stats_ = stats_[:, 4]
...
def generateFeatureDescriptors(I, x, y, patch_size=21):
    ...
    for i in range(num_points):
        patch = extract_bordered_patch(
            I, x[i], y[i], patch_size, patch_size, border=patch_size)
...
def eval_TFPN_s(kp1, kp2, selected_matches, all_matches, trans_matrix, judge_threshhold=16):
    ...
    for single_match in all_matches:
        ...
        distance = np.linalg.norm(
            np.array(pos_kp2)-np.array(pos_kp1_))  # type: ignore
...
def eval_TFPN(kp1_x, kp1_y, kp2_x, kp2_y, Dist, Matchlist, trans_matrix, judge_threshhold=16):
    ...
    for queryIdx, trainIdx in Matchlist:
        ...
        if distance < judge_threshhold:
            TP += 1
            single_match = [[kp1_x[queryIdx], kp1_y[queryIdx]],
                            [kp2_x[trainIdx], kp2_y[trainIdx]]]
            TP_matches.append(single_match)
...
def print_Rarray(arrayname, number_array):
    str_array = arrayname+': '
    variable_names = ['P', 'N', 'TP', 'FN',
                      'FP', 'TN', 'TPR', 'FPR', 'PPV', 'ACC']
    for i in range(0, 6):
...
def draw_correspondences(image1, image2, points, color1, color2):
    ...
    for p in points:
        image = cv2.line(image, tuple(p[0]), tuple(
            p[1] + [image1.shape[1], 0]), color2, thickness)
...
def post_Distance2Matches_s(MatchList, All_MatchList, kp1, kp2, img_o, img_t, trans_matrix):
    ...
    result_array_d, TP_matches = eval_TFPN_s(
        kp1, kp2, MatchList, All_MatchList, trans_matrix)

    img_matches = cv2.drawMatches(img_o, kp1, img_t, kp2, TP_matches,
...
H1to2p = np.loadtxt(folder+'H1to2p')
H1to3p = np.loadtxt(folder+'H1to3p')
H1to4p = np.loadtxt(folder+'H1to4p')
H1to5p = np.loadtxt(folder+'H1to5p')
H1to6p = np.loadtxt(folder+'H1to6p')

HH_loop = [(H1to2p, 'H2p_'), (H1to3p, 'H3p_'), (H1to4p, 'H4p_'),
           (H1to5p, 'H5p_'), (H1to6p, 'H6p_')]
for TMatrix, Hid in HH_loop:
    # Apply transformation matrix to image
    sub_folder = folder+Hid
    print(sub_folder)
    img_t = cv2.warpPerspective(
        img_o2, TMatrix, (img_o2.shape[1], img_o2.shape[0]))
    ...
    x_img_o, y_img_o, mask_img_o = FeatureMeasure2Points(
        R_img_o, num_points, 0.8)
    x_img_t, y_img_t, mask_img_t = FeatureMeasure2Points(
        R_img_t, num_points, 0.8)

    img_o_save1 = img_o3.copy()
    img_t_save1 = img_t.copy()

    for i in range(len(x_img_o)):
        cv2.circle(img_o_save1, (int(x_img_o[i]), int(
            y_img_o[i])), 3, marker_color, -1)

    for i in range(len(x_img_t)):
        cv2.circle(img_t_save1, (int(x_img_t[i]), int(
            y_img_t[i])), 3, marker_color, -1)
    ...
    Dlist_img_o = generateFeatureDescriptors(
        gray_o, x_img_o, y_img_o, patch_size)
    img_o_save2 = show_descriptor_image(
        img_o_save1.copy(), x_img_o, y_img_o, Dlist_img_o, patch_size)

    Dlist_img_t = generateFeatureDescriptors(
        gray_t, x_img_t, y_img_t, patch_size)
    img_t_save2 = show_descriptor_image(
        img_t_save1.copy(), x_img_t, y_img_t, Dlist_img_t, patch_size)
    ...
    threshold_match_d = 3.5
    threshold_match_n = 10.2
    threshold_match_nn = 0.8
    ...
    result_array_d, TP_matches_d = eval_TFPN(
        x_img_o, y_img_o, x_img_t, y_img_t, Dist, MatchList1, TMatrix)
    result_array_n, TP_matches_n = eval_TFPN(
        x_img_o, y_img_o, x_img_t, y_img_t, Dist, MatchList2, TMatrix)
    result_array_nn, TP_matches_nn = eval_TFPN(
        x_img_o, y_img_o, x_img_t, y_img_t, Dist, MatchList3, TMatrix)

    result_image_d = draw_correspondences(
        img_o_save1.copy(), img_t_save1.copy(), TP_matches_d, (0, 0, 255), (0, 255, 0))
    result_image_n = draw_correspondences(
        img_o_save1.copy(), img_t_save1.copy(), TP_matches_n, (0, 0, 255), (0, 255, 0))
    result_image_nn = draw_correspondences(img_o_save1.copy(
    ), img_t_save1.copy(), TP_matches_nn, (0, 0, 255), (0, 255, 0))
    ...
    threshold_match_d = 120
    threshold_match_n = 0.6
    threshold_match_nn = 0.7
    MatchList1_s = Distance2Matches_DistThresh_s(All_Matchs, threshold_match_d)
    MatchList2_s = Distance2Matches_NearestMatch_s(
        All_Matchs, threshold_match_n)
    MatchList3_s = Distance2Matches_NearestRatio_s(
        All_Matchs, threshold_match_nn)

    result_array_s_d, TP_matches_s_d = eval_TFPN_s(
        kp1, kp2, MatchList1_s, All_Matchs, TMatrix)
    result_array_s_n, TP_matches_s_n = eval_TFPN_s(
        kp1, kp2, MatchList2_s, All_Matchs, TMatrix)
    result_array_s_nn, TP_matches_s_nn = eval_TFPN_s(
        kp1, kp2, MatchList3_s, All_Matchs, TMatrix)

    result_image_s_d = draw_correspondences(
        img_o3.copy(), img_t.copy(), TP_matches_s_d, (0, 255, 0), (0, 0, 255))
    result_image_s_n = draw_correspondences(
        img_o3.copy(), img_t.copy(), TP_matches_s_n, (0, 255, 0), (0, 0, 255))
    result_image_s_nn = draw_correspondences(
        img_o3.copy(), img_t.copy(), TP_matches_s_nn, (0, 255, 0), (0, 0, 255))
    ...
    Result = [result_array_d, result_array_n, result_array_nn,
              result_array_s_d, result_array_s_n, result_array_s_nn]
